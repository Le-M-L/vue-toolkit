...
export function useAntdDesignTokens(preferences: Preferences) {
...
export function useNaiveDesignTokens(preferences: Preferences) {
  const rootStyles = getComputedStyle(document.documentElement);
...
export function useElementPlusDesignTokens(preferences: Preferences) {
  ...
  watch(
    ...
    () => {
      ...
      executeUpdateCSSVariables(variables, `__ymm_design_styles__`);
    },
    { immediate: true },
  );
}
